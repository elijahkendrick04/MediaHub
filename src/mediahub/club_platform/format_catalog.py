...
_CATALOG: tuple[FormatSpec, ...] = (
    ...
    FormatSpec(
        "ig_post", "Instagram post", "social_size", 1080, 1350,
        render_format="feed_portrait",
        description="Portrait 4:5 in-feed post — the highest-reach feed size.",
        caption_style="feed",
    ),
    FormatSpec(
        "ig_square", "Instagram square", "social_size", 1080, 1080,
        render_format="feed_square",
        description="Classic 1:1 feed post.",
        caption_style="feed",
    ),
    FormatSpec(
        "ig_story", "Instagram / Reel story", "social_size", 1080, 1920,
        render_format="story",
        description="Full-screen 9:16 story or reel cover.",
        safe_top=250, safe_bottom=250,
        caption_style="story",
    ),
    FormatSpec(
        "ig_reel_cover", "Reel cover", "social_size", 1080, 1920,
        render_format="reel_cover",
        description="9:16 cover frame for a reel, designed for the grid crop.",
        safe_top=250, safe_bottom=420,
        caption_style="story",
    ),
    FormatSpec(
        "fb_post", "Facebook post", "social_size", 1080, 1080,
        render_format="feed_square",
        description="Square Facebook feed post.",
        caption_style="feed",
    ),
    FormatSpec(
        "fb_cover", "Facebook cover", "social_size", 1640, 624,
        description="Facebook Page cover banner (16:6-ish).",
        caption_style="banner",
    ),
    FormatSpec(
        "fb_event_cover", "Facebook event cover", "social_size", 1920, 1005,
        description="Facebook event header.",
        caption_style="banner",
    ),
    FormatSpec(
        "x_post", "X / Twitter post", "social_size", 1600, 900,
        description="16:9 in-feed image for X / Twitter.",
        caption_style="feed",
    ),
    FormatSpec(
        "x_header", "X / Twitter header", "social_size", 1500, 500,
        description="X / Twitter profile header (3:1).",
        caption_style="banner",
    ),
    FormatSpec(
        "linkedin_post", "LinkedIn post", "social_size", 1200, 1200,
        description="Square LinkedIn feed post.",
        caption_style="feed",
    ),
    FormatSpec(
        "linkedin_banner", "LinkedIn banner", "social_size", 1584, 396,
        description="LinkedIn Page / profile cover (4:1).",
        caption_style="banner",
    ),
    FormatSpec(
        "pinterest_pin", "Pinterest pin", "social_size", 1000, 1500,
        description="Tall 2:3 Pinterest pin.",
        caption_style="feed",
    ),
    FormatSpec(
        "tiktok_video", "TikTok cover", "social_size", 1080, 1920,
        render_format="story",
    ...
    FormatSpec(
        "youtube_thumbnail", "YouTube thumbnail", "social_size", 1280, 720,
        description="16:9 YouTube thumbnail.",
        caption_style="banner",
    ),
    FormatSpec(
        "youtube_banner", "YouTube banner", "social_size", 2560, 1440,
        description="YouTube channel art; key content sits in the 1546×423 safe area.",
        safe_top=508, safe_bottom=509, safe_left=507, safe_right=507,
        caption_style="banner",
    ),
    # --- Carousel ----------------------------------------------------------
    FormatSpec(
        "carousel_slide", "Carousel slide", "carousel", 1080, 1080,
    ...
    FormatSpec(
        "poster", "Poster", "poster", 1080, 1350,
    ...
    FormatSpec(
        "certificate", "Certificate", "certificate", 1754, 1240,
    ...
    FormatSpec(
        "athlete_one_pager", "Athlete one-pager", "document", 1240, 1754,
    ...
    FormatSpec(
        "season_calendar", "Season calendar", "calendar", 1080, 1350,
    ...
    FormatSpec(
        "club_wallpaper_phone", "Phone wallpaper", "wallpaper", 1080, 1920,
        render_format="story",
    ...
    FormatSpec(
        "club_wallpaper_desktop", "Desktop wallpaper", "wallpaper", 1920, 1080,
    ...
    *,
    ...
    for v in (w_px, h_px):
        if v < _MIN_PX or v > _MAX_PX:
            raise ValueError(
                f"custom size {w_px}×{h_px}px out of range "
                f"({_MIN_PX}–{_MAX_PX}px per side)"
            )
    return FormatSpec(
