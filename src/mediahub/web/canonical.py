...
class Swimmer:
    ...
    swimmer_key: (
        str  # primary internal key (asa_id when known, else "club:lname,fname:dob" or stable hash)
    )
    first_name: str
    last_name: str
    gender: str  # 'M' | 'F' | 'X' | ''
    ...
    identity_confidence: str = (
        "high"  # high (id-matched) | medium (name+club+dob) | low (name only)
    )
...
class Meet:
    ...
    )  # which fields were inferred not parsed
