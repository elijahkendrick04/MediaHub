...
def _finalise(
    ...
    *,
    ...
    # silent fallback) + the poster-frame sidecar, then the explainability
    # manifest (M22 — the ffmpeg engine writes the same sidecar shape the
    # Remotion paths write, so an ffmpeg MP4 is never an unexplained
    # "legacy render"), then publish (which ships the sidecars along).
