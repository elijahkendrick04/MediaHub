...
def _ken_burns_filter(duration_sec: float, *, variant: str = "zoom_in", tag: str = "0") -> str:
...
def story_ffmpeg_args(
    ...
    vf = (
        f"{_ken_burns_filter(duration_sec, variant=variant, tag='s')},"
...
def reel_ffmpeg_args(
    ...
    *,
    ...
    for i, dur in enumerate(segment_durations):
        chains.append(
            f"[{i}:v]{_ken_burns_filter(dur, variant=kb_variants[i], tag=str(i))},setsar=1[v{i}]"
        )
    last = "v0"
    elapsed = 0.0
