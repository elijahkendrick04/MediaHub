...
- **Quotas** — counts how much AI each club uses and can put a cap on it. By
  ...
  routes are wired through the meter (the `context.feature_scope` guard, or the
  direct `quota.reserve`/`quota.finalize` calls the caption and translate
  routes use today). The raw counting lives next door in
