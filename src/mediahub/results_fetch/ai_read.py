...
_PROMPT_TEMPLATE = (
    ...
    f"{_TABLE_SEP}\n"
    "EVENT ASSOCIATION IS CRITICAL: the event name is usually a heading ABOVE a "
    "results table (e.g. 'Event 12 Mens 100m Breaststroke'). Copy that heading "
    "VERBATIM — exact distance, stroke and gender — into the 'event' column for "
    "EVERY row that sits under it, and start a NEW table at each new heading. "
    "Never carry one table's event onto another table's rows, never guess the "
    "event from a swimmer's time, and never merge two events into one table. If a "
    "result's heading is unclear, leave 'event' blank rather than guessing.\n"
