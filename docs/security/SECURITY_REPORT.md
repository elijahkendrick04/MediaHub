...
| **V4 Access control** | 4.1/4.2 per-record authz | Tenant isolation on run routes (`_can_access_run`, 37 routes), media library, brand kits, consent registry, DSR requests, retention/child-policy settings | `test_run_route_isolation_invariant.py`, `test_cross_tenant_access.py`, `test_media_library_profile_isolation.py`, `test_compliance_tenant_isolation.py` |
...
| **V11 Business logic** | 11.1.5 | **Human approval + export-only**: MediaHub has no machine path that places content on a social account — no schedule route, no autonomous publishing; approved cards are exported/downloaded for manual posting. The approval route enforces the consent gate server-side: the consent answer unifies the W.2 safeguarding levels AND the compliance ledger (blocked if either blocks, fail-closed), with unknown-age treated as a minor | `test_consent_gating.py`, `test_llm_pipeline_security.py` |
...
| R3 | **Social engineering of club operators** (credential phishing, rogue volunteer) | 2FA available; security event log for forensics; consent gate limits what any operator can approve/export about opted-out children; club-side controls are the club's duty (DPA) |
| R4 | **Malicious authorised insider** exporting content they are allowed to export (and posting it manually) | Out of technical scope; audit ledger + approval records give attribution |
...
- Full pytest suite: **green** — 4,465 passed, 1 environment skip,
  0 failures (2026-06-12, post-merge with the PR #352 / Phase W
  baseline), including ~120 new compliance/security tests.
