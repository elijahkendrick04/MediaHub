...
| F6 | Measured layout scoring (whitespace band, centroid balance, collision reject) picking among K candidate packs. | Deterministic evaluate()-based scorer over the seeded candidate walk. | **[SHIPPED]** (`graphic_renderer/layout_score.py` — O'Donovan-style energy scorer over the current pack + a ground-diverse strided sample of its pool (catalog or mood bundle); a `getBoundingClientRect` sweep per candidate scores text-collision hard-reject + whitespace band + importance-weighted centroid + edge alignment + saliency clearance, ships the argmax. One decision per card at the 1080×1350 certification anchor, persisted onto the brief so every cut and the motion mirror wear the same winner; geometry cached beside the G1.24 PNG cache; switch recorded on the safety-notes trail. Opt-in `MEDIAHUB_LAYOUT_SCORE`, byte-identical off, switches only on a measured win) |
