...
| `7abbd25` | `mediahub.privacy` package: run-deletion cascade (per-run PB cache, caption memory, motion cache); athlete erasure across runs/assets/caches/memory/log with prose redaction; account deletion (`UserStore.delete`, `MembershipStore.erase_email` — physical, tombstone-free) + password-verified route; account export | 1.6, 1.7, C1, C2 |
...
3. **Vendor data-processing terms + transfer mechanisms** — accept/execute and record
   for: Google (Gemini API), Anthropic, Photoroom, Replicate, Stripe, ntfy
...
9. **Breach runbook**: name the person who notifies clubs without undue delay and
   the ICO within 72h; the autonomy audit ledger is the evidence source.
...
export-only output with no social publishing, deterministic accuracy engine) is now
matched by a legal wrapper that describes reality and is enforced by tests (~75 new
...
weakening the human-approval or consent gating (don't), before adding any non-essential
