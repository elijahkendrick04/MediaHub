...
The graphic still-renderer and the Remotion/ffmpeg reel engine each get a 30-item deep-upgrade
sprint — **30 graphic builds (`G1.*`)** and **30 reel builds (`R1.*`)**, listed *first* in the
build queue below. Every item is deliberately sized to be a **whole session's work on its own** —
too large to safely co-build with another in the same session (it would time out, or each half
would land weaker) — and no two items build the same feature. Each is scoped to a **distinct
module / function / layout file** and names its primary surface, so all 60 can be dispatched as
**independent parallel sessions on their own branches**. The only places two items meet are small,
mechanical registration tables — `style_packs.py`, `archetypes.py`, the scene/intent switch in
`remotion/src/compositions/StoryCard.tsx`, and `creative_brief/design_spec.py` — which resolve
trivially on merge; nothing depends on another item's runtime behaviour, so dispatch order is free.
The **deterministic-engine boundary stays untouched** (parsers, detectors, ranker, colour-science) —
these are renderer/compositor/audio builds only. 🟢 = parallel-safe.
...
<!-- ROADMAP:TODO -->
- **G1.1** · Graphic generator sprint — Eight new v2 archetype families (timeline-progression, radial competition ring, vertical stat-tower, three-card editorial grid, staggered-diagonal offset, full-height portrait split, ribbon-banner, contact-sheet). Build each as a new `layouts/v2/<name>.html` + `.notes.md` using only role tokens (`--mh-*`) and autofit vars; they auto-register through `archetypes.list_archetypes()`. *Primary: graphic_renderer/layouts/v2/, archetypes.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.2** · Graphic generator sprint — Multi-athlete collage / relay layout engine: composite 2–4 cutouts into one balanced frame with deterministic multi-subject placement. New `graphic_renderer/collage.py` + a `relay_collage` layout + multi-crop saliency packing. *Primary: new collage.py, layouts/v2/relay_collage.html* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.3** · Graphic generator sprint — Landscape & extended aspect-ratio support (16:9, 3:2, 4:3) with per-format composition rules. Extend `FORMAT_SIZES`, add format-aware layout tuning, thread through `variants.render_all_formats`. *Primary: render.py (FORMAT_SIZES), variants.py, layouts CSS* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.4** · Graphic generator sprint — Ground-treatment expansion pack: add gradient-mesh, bokeh, light-ray and paper-weave grounds to `style_packs.GROUNDS` with overlay HTML and visual-weight caps + tests. *Primary: style_packs.py (GROUNDS)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.5** · Graphic generator sprint — Accent-geometry expansion pack: hexagons, art-deco corners, wave rules, spiral flourishes, glitch dividers in `style_packs.ACCENT_GEOS` + `pack_overlay_html`. *Primary: style_packs.py (ACCENT_GEOS)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.6** · Graphic generator sprint — Texture-layering engine: stack two textures with blend modes (grain+dots, halftone+weave…) via a new compositor over `style_packs.TEXTURES`, weight-capped. *Primary: style_packs.py (TEXTURES + compositor)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.7** · Graphic generator sprint — Photo-derived palette extraction + ground tinting: PIL k-means dominant/secondary colours from the athlete photo, fed as an APCA-gated tint into the ground layer only (never overrides confirmed brand hex). New `graphic_renderer/photo_palette.py`. *Primary: new photo_palette.py + render.py hook* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.8** · Graphic generator sprint — Gradient-mesh background engine: deterministic multi-stop linear/radial/conic meshes keyed to brand roles, emitted as SVG data URIs. New `graphic_renderer/gradient_mesh.py`. *Primary: new gradient_mesh.py + background hook* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.9** · Graphic generator sprint — Variable-font support: integrate variable WOFF2 axes (weight/width/optical size) and per-slot axis optimisation in autofit. *Primary: layouts/_shared.css, layouts/fonts/, autofit.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.10** · Graphic generator sprint — Club custom-font upload pipeline: security-sandboxed validation, subsetting and first-party self-hosting of an uploaded brand face. New `typography/font_intake.py` + brand-kit wiring. *Primary: new font_intake.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.11** · Graphic generator sprint — Kerning/ligature-aware autofit: extend the char-width tables with kerning-pair corrections and common ligatures for tighter, truer measured fits. *Primary: autofit.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.12** · Graphic generator sprint — Multi-line hero & split-result fitting: wrap and balance compound surnames and split times (e.g. `1:45.23 / 50.12`) with deterministic auto-size. *Primary: autofit.py + v2 hero slots* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.13** · Graphic generator sprint — SVG vector export path: emit an editable SVG (outlined fonts, no raster fallback) alongside each PNG. New `graphic_renderer/svg_export.py`. *Primary: new svg_export.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.14** · Graphic generator sprint — WebP/AVIF output + render quality profiles (fast/standard/high DPR): extend the renderer encode step and a profile config. *Primary: render.py (render_html_to_png output)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.15** · Graphic generator sprint — Batch ZIP export of a content pack's every format + a `metadata.json` manifest (athlete, event, archetype, pack, colours). New `graphic_renderer/pack_export.py` + a download route. *Primary: new pack_export.py + web route* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.16** · Graphic generator sprint — EXIF/IPTC metadata + photographer-attribution embedding in exported PNG/JPEG. New `graphic_renderer/metadata_embed.py`. *Primary: new metadata_embed.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.17** · Graphic generator sprint — Print pipeline expansion: trim/bleed/crop marks + CMYK-aware export for professional printing, extending the A4 print layouts. *Primary: print_export.py + print layouts* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.18** · Graphic generator sprint — Colour accessibility audit + colourblind simulation: per-card APCA/WCAG report and deuteranopia/protanopia/tritanopia preview renders. New `quality/colour_audit.py`. *Primary: new colour_audit.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.19** · Graphic generator sprint — Grayscale/mono accessibility render mode: a deterministic B/W path with role remap for print/low-ink and accessibility. *Primary: render.py mode + layouts CSS* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.20** · Graphic generator sprint — Brand-palette expansion to N custom club colours with APCA-gated automatic role assignment (beyond primary/secondary/accent). *Primary: theming/ (roles, palette) + render role injection* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.21** · Graphic generator sprint — Depth-of-field background-blur photo treatment: subtle focus on the athlete, softened background, as a new `photo_treatment` value. *Primary: render.py (_photo_treatment_css) + CSS* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.22** · Graphic generator sprint — Icon/badge overlay system: deterministic medal / flag / club-record / ribbon SVG icon placement layer. New `graphic_renderer/icon_overlay.py` + a curated icon set. *Primary: new icon_overlay.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.23** · Graphic generator sprint — Headless-Chromium context pooling: reuse a warm browser context across renders instead of launch-per-render, for far faster batch/multi-format runs. *Primary: render.py renderer infra + variants.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.24** · Graphic generator sprint — Incremental render-stage caching: hash and cache the HTML/CSS/asset-URI stages so re-renders skip unchanged work. New cache layer in the renderer. *Primary: render.py + new cache module* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.25** · Graphic generator sprint — Server-side photo adjustment stack: deterministic PIL recipes (sharpen, contrast, saturation, levels) applied before inlining. New `graphic_renderer/photo_adjust.py`. *Primary: new photo_adjust.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.26** · Graphic generator sprint — Archetype × style-pack gallery UI: browse every archetype and pack with live preview thumbnails + category filters before generating. New web route + template. *Primary: web/web.py route + template* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.27** · Graphic generator sprint — Interactive brief/design editor: tweak text layers, palette, archetype and style pack with live re-render in the browser. New web route + UI. *Primary: web/web.py route + template* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.28** · Graphic generator sprint — Mood-keyed style-pack preset bundles: curated, deterministic pack bundles selected from `brief.mood`. New preset-map functions in style_packs. *Primary: style_packs.py (new preset functions)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.29** · Graphic generator sprint — Animated still loops: export subtle SVG-animation backgrounds as APNG/GIF for static-but-living posts. New `graphic_renderer/animated_still.py`. *Primary: new animated_still.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **G1.30** · Graphic generator sprint — Render debug/inspection overlay + design-explainability sidecar: crop box, saliency centroid, fitted font sizes, and a "why this archetype/pack/colour" JSON, behind an opt-in render mode. New `graphic_renderer/inspect.py`. *Primary: new inspect.py + render.py hook* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.1** · Reel generator sprint — Motion-intent program pack: add 5+ new intents (bounce_in, flip_reveal, swirl, reveal_from_sides, cascade) to `animProgram()`, extend the `MOTION_INTENTS` enum, document in motion-craft, and add parity tests. *Primary: StoryCard.tsx (animProgram), creative_brief/design_spec.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.2** · Reel generator sprint — Scene-mode pack: add 3 new scenes (vertical-split, radial-rings, marquee-crawl) to the StoryCard scene system + archetype→scene map, with still-renderer parity. *Primary: StoryCard.tsx (scenes, sceneForArchetype)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.3** · Reel generator sprint — Subtitle/caption burn-in engine: read the voiceover SRT and render APCA-gated on-video captions. New `visual/subtitle_burn.py` + a StoryCard caption layer. *Primary: new subtitle_burn.py + StoryCard.tsx* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.4** · Reel generator sprint — Motion background-pattern expansion: 6 new frame-pure animated SVG patterns (checkerboard, diamonds, circuit, organic-waves…) in `bgPatternFor`/`PatternLayer`. *Primary: StoryCard.tsx (bgPatternFor, PatternLayer)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.5** · Reel generator sprint — Motion accent-decoration expansion + sizing variants (thick/thin stripe, large/small brackets, offset badge) in `accentDecoration()`. *Primary: StoryCard.tsx (accentDecoration)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.6** · Reel generator sprint — Animated-pattern drift layer: per-pattern frame-pure motion (rotate/opacity/scale) during the breathe phase, no `Math.random`. *Primary: StoryCard.tsx (PatternLayer)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.7** · Reel generator sprint — Format-aware photo focal points: compute per-format (story/portrait/square/landscape) saliency crops and feed them into motion props. *Primary: visual/motion.py (_photo_focus_for_brief) + saliency.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.8** · Reel generator sprint — Photo scrim variant system for motion: edge / radial / gradient / corner scrims for legibility, role-driven. *Primary: StoryCard.tsx (PhotoLayer)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.9** · Reel generator sprint — Cutout-layer compositing in motion: render the alpha cutout as a separate animated foreground layer with parallax over the background. *Primary: StoryCard.tsx (PhotoLayer) + motion.py asset prep* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.10** · Reel generator sprint — Motion photo filter stack: deterministic frame-pure brightness/contrast/saturation/blur driven by brief fields. *Primary: StoryCard.tsx (PhotoLayer filters) + brief* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.11** · Reel generator sprint — On-video text-effect library: glow, outline, 3D-shadow, stroke-animate, blur-to-focus as pure frame functions. *Primary: StoryCard.tsx (text components)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.12** · Reel generator sprint — Reel beat-rhythm & duration customisation: per-card beat weights plus custom cover/outro durations, validated against `reel_duration_for`. *Primary: MeetReel.tsx (beat carving) + visual/motion.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.13** · Reel generator sprint — Custom reel stat chips: configurable honest stat labels (top splits, season records, relay wins) counted from card facts, never invented. *Primary: MeetReel.tsx (reelStats, StatChips)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.14** · Reel generator sprint — Transition library expansion + per-card timing: add new transitions (glitch, slide-stack, light-sweep) and a `transition_duration` brief field, capped to the motion-craft <400ms rule. *Primary: MeetReel.tsx (transitions)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.15** · Reel generator sprint — Multi-format batch reel render: one request renders and caches all four cuts (story/portrait/square/landscape) in a single pass. *Primary: visual/motion.py (render_meet_reel) + web route* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.16** · Reel generator sprint — ffmpeg fallback engine multi-format support: render portrait/square/landscape (today it raises `ReelEngineUnavailable` for all but story). *Primary: visual/reel_ffmpeg.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.17** · Reel generator sprint — ffmpeg fallback engine richer motion: Ken Burns variants, parallax and beat transitions that mirror the Remotion cuts so the free engine looks intentional. *Primary: visual/reel_ffmpeg.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.18** · Reel generator sprint — Narration script-style templates (verbose / compact / poetic / technical) that stay strictly fact-only over the verified facts. *Primary: visual/narration.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.19** · Reel generator sprint — Per-card audio-mix profiles (voice_lead / balanced / music_forward) folded into the audio plan and cache key. *Primary: visual/audio_mux.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.20** · Reel generator sprint — Music-bed upgrade: refined automatic ducking, intro/outro stings, and beat-aware alignment of card cuts to the track. *Primary: visual/audio_mux.py + music-pool logic* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.21** · Reel generator sprint — Local Piper TTS provider backend behind the existing voiceover seam (zero-cost narration; honest error if the model is absent). *Primary: visual/voiceover.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.22** · Reel generator sprint — Colour-role transition animation across the clip (fade / gradient / pulse between roles), APCA-safe at every frame. *Primary: StoryCard.tsx (role handling)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.23** · Reel generator sprint — Dynamic logo sizing + animated logo reveals per scene (small/medium/large/auto, scene-aware entrance). *Primary: StoryCard.tsx (LogoChip)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.24** · Reel generator sprint — Ambient motion programmes: slow drift / pan / temperature shift sustained through the breathe phase as a new `ambient_motion` axis. *Primary: StoryCard.tsx + brief* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.25** · Reel generator sprint — Multi-part stagger sequences: component-group cascades (name → event → result → chips) beyond the current per-word `kinetic_type`. *Primary: StoryCard.tsx (stagger system)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.26** · Reel generator sprint — Easing-curve customisation + expanded mood→spring vocabulary (melancholic, energetic, victorious, contemplative…). *Primary: StoryCard.tsx (springConfigFor)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.27** · Reel generator sprint — Frame-by-frame motion visual-regression harness: render reference frames, pixel-diff in CI to catch silent layout/motion drift. New tooling + tests. *Primary: tests/ + new harness script* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.28** · Reel generator sprint — Parallel reel composition: render the card beats concurrently via a subprocess farm and composite the final cut, cutting wall-clock time. *Primary: visual/motion.py (render_meet_reel)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.29** · Reel generator sprint — Progressive in-render poster capture via a Remotion `delayRender` hook, skipping the separate ffprobe poster extraction. *Primary: remotion/render.js + visual/motion.py* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
- **R1.30** · Reel generator sprint — Reel cover & outro redesign system: data-driven cover variants + outro CTA variants (follow handle, next meet, sponsor thanks), all from verified facts. *Primary: MeetReel.tsx (cover, outro)* · ❌ **NOT STARTED** · 🟢 PARALLEL-SAFE
