...
| A7 | **Publication relay** (per-club opt-in; human approval default) | Athletes featured on approved cards | Approved card image + caption (name, photo) | On club approval, content is exported/relayed to social platforms; platforms become independent controllers | Buffer (if enabled); Meta/TikTok as independent controllers | UK→US | Unbypassable approval gate; publish gate (kill switch, safeguarding: minors never auto-publish); immutable audit ledger |
