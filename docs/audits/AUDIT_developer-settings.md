...
- **Third pass (F-4) — the operator asked for the remaining caveats to be closed.** The one actionable caveat (F-4, `/healthz/deps` path disclosure) is now fixed: absolute paths are operator-only, monitoring booleans preserved. Code+test commit (the redaction) + report commit; new test `tests/test_healthz_deps_path_disclosure.py`. Branch restarted from the latest `origin/main` (the earlier developer-settings branch was already merged, so this pass is a fresh change on top of current `main`), rebased onto BASE `2349a41`.
  - **Green gate (before fixing the blocker):** app boots clean (483 routes); the feature+affected subset (189 tests) all pass; full `tests/` suite = **12,546 passed / 10 skipped / 5 failed**, where the 5 were pre-existing and unrelated: 4× `test_spotlight_build_brand_grounding` (flaky under parallel xdist — pass in isolation) and 1× `test_theme_tokens::test_inline_hex_count_within_budget` (deterministic, but failed identically on a clean `origin/main` worktree; my diff adds zero hex).
- **Fourth pass — fix the blocker + merge on green (operator-directed).** Because `main` was red on the CI-gating `test_inline_hex_count_within_budget` (21 vs 20) — blocking a clean merge — the operator directed *"find out why it's not green, fix the issue then merge on green."* Fixed by dropping the three dead `#FFB454` fallbacks (`→ var(--warn)`, provably zero-visual-change), taking the count to **18**. See Cross-cutting changes / section 8.
  - **Merge status: MERGED to `main`.** Integrated the moving `main` across the run (BASE `2349a41` → `db8de26` [video-studio] → `7b545e4` [org-access]; rebased cleanly each time), re-ran the green gate on each integrated result, passed the freshness re-check (`origin/main` == BASE `7b545e4` at push time), and landed via a fast-forward `git push origin HEAD:main`. Merged commits on `main` above `7b545e4`: `447bfce` (F-4 redaction + test), `d94720c` (report), `54b9db3` (theme-budget fix + report); `main` tip after push: **`54b9db3`**.
  - **Final green gate (integrated onto `7b545e4`):** app boots clean (483 routes); hex-budget count **18 → PASS**; 313-test feature+theme+auth+org-gate+settings+api-docs regression subset passed; full `tests/` suite on the prior integration = **12,550 passed / 10 skipped / 8 failed**, where all 8 are flaky/environmental (spotlight ×4, cutout-preview ×3, one password-reset async-timing test) and **all 8 pass in isolation** (17 passed) — none in the developer-settings area, none caused by this diff; the previously-red hex-budget test now passes. ruff (pinned v0.8.4) lint + format clean; no secrets or `.env` staged.
  - **Review the diff:** `git diff 7b545e4..54b9db3` (or `git show 447bfce` for the F-4 code fix alone).
