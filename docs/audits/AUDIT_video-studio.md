...
| F-05 | P3 | "Make clip" allows double-submit (duplicate projects) | Double-click "Make clip" → two clip-maker calls → two projects. | Handler didn't disable the button (the reel handler did). | **FIXED** — button disabled on click, re-enabled on settle. | (this branch) |
