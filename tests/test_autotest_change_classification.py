...
def test_pure_product_change_is_product():
    assert builder.classify_change(["src/mediahub/web/web.py"]) == "product"
    assert builder.classify_change(["src/mediahub/web/web.py", "tests/test_web.py"]) == "product"


def test_harness_change_is_harness():
    assert builder.classify_change(["autotest/fix_loop.py"]) == "harness"
    assert builder.classify_change(["autotest/reports/ledger.json"]) == "harness"


def test_workflow_and_deploy_are_harness():
    assert builder.classify_change([".github/workflows/ci.yml"]) == "harness"
    assert builder.classify_change(["render.yaml"]) == "harness"
    assert builder.classify_change(["Dockerfile"]) == "harness"
    assert builder.classify_change(["pyproject.toml"]) == "harness"
    assert builder.classify_change(["requirements.txt"]) == "harness"


def test_governance_docs_are_harness():
    assert builder.classify_change(["CLAUDE.md"]) == "harness"
    assert builder.classify_change(["autotest/CHANGE_CLASSIFICATION.md"]) == "harness"
    assert builder.classify_change(["autotest/PROOF_CRITERION.md"]) == "harness"


def test_mixed_diff_is_harness_stricter_wins():
    assert builder.classify_change(["src/mediahub/web/web.py", "autotest/x.py"]) == "harness"


def test_empty_set_fails_safe_to_harness():
    # An unknown/empty change must never auto-merge.
    assert builder.classify_change([]) == "harness"
...
def test_merge_gate_blocks_harness_automerge(monkeypatch):
    ...
    msg = builder._merge_to_main("b", has_pr=True, files=["autotest/fix_loop.py"])
