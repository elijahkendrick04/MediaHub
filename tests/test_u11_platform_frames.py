...
# =========================================================================== #
# The distinctive class tokens of the removed flat sample-output-card component.
# (mh-sample-cta is a *different*, pre-existing class — the bundled-sample CTA
# button — so we match the component's own tokens, never the bare "mh-sample"
# substring, which would false-positive on it.)
_REMOVED_SAMPLE_TOKENS = (
    "mh-sample-row",
    "mh-sample-eyebrow",
    "mh-sample-title",
    "mh-sample-meta",
...
def test_old_sample_component_removed_from_page(home):
    for tok in _REMOVED_SAMPLE_TOKENS:
        assert tok not in home, tok


def test_old_sample_css_removed():
    for tok in _REMOVED_SAMPLE_TOKENS:
        assert tok not in COMPONENTS_CSS, tok
...
def test_no_stray_sample_refs_in_tree():
    ...
    for ext in ("*.py", "*.css", "*.js"):
        for f in root.rglob(ext):
            ...
            for tok in _REMOVED_SAMPLE_TOKENS:
                if tok in text:
                    hits.append(f"{f.name}: {tok!r}")
    assert not hits, f"stray removed-sample-component refs: {hits}"
