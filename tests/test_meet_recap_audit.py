...
@pytest.fixture
def env(web_module, client):
    # bypass the org-ready gate (conftest convention): the `app` fixture behind
    # `client` already sets app.config["TESTING"] = True.
...
def _run_meet(c, wm, club):
    ...
    r = c.post(
        "/upload",
        data={"file": (io.BytesIO(data), "results_hy3.zip")},
...
class TestUploadRejectStatus:
    ...
    def test_empty_file_is_400(self, env):
        c, _ = env
        r = c.post(
            "/upload", data={"file": (io.BytesIO(b""), "e.hy3")}, content_type="multipart/form-data"
        )
        assert r.status_code == 400
...
class TestReconfigurePersistedRun:
    def test_configure_rebuilds_meta_from_saved_input(self, env):
        ...
        (d / "resume.json").write_text(
            json.dumps(
                {
                    "file_name": "results_hy3.zip",
                    "profile_id": "cog",
                    "use_pb_cache": True,
                    "fetch_pbs": False,
                    "club_filter": "City Of Glasgow Swim Team",
                    "source_url": None,
                }
            )
        )
