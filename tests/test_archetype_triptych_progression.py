"""Pinning test for the PAR-7 ``triptych_progression`` v2 archetype.
...
is supplied, wraps its hero text so a long/space-less name can never clip, and
assembles into clean HTML through the real ``render_brief`` path (Playwright
stubbed) — i.e. it actually picks up the injected brand-role and autofit tokens
like every other v2 archetype. Also pins that adding this archetype lifts a
representative seeds-0..9 pack's archetype-diversity above the prior high-water
mark (the targeted §8C structural-distinctiveness metric).
"""
...
def test_optional_hero_chip_collapses_when_empty():
    assert ".tp__hero:has(.tp__hero-val:empty)" in _raw()
...
def test_uses_three_equal_bays():
    raw = _raw()
    assert raw.count('class="tp__bay') == 3
    assert "flex: 1 1 0" in raw
