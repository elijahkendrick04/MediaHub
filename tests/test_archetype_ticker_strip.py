"""Pinning test for the PAR-7 ``ticker_strip`` v2 archetype.
...
via ``--mh-*`` roles), collapses its optional highlight chip when no hero stat
is supplied, and assembles into clean HTML through the real ``render_brief``
path (Playwright stubbed) — i.e. it actually picks up the injected brand-role
and autofit tokens like every other v2 archetype. Also pins that adding this
archetype lifts a representative seeds-0..9 pack's archetype-diversity above the
prior high-water mark (the targeted §8C structural-distinctiveness metric).
"""
...
def test_optional_hero_chip_collapses_when_empty():
    # the highlight block must hide itself when no hero stat is supplied, so an
    # empty slot never leaves a dangling accent rule on the foot.
    assert ".tk__hero:has(.tk__hero-val:empty)" in _raw()


def test_hero_text_wraps_not_clips():
    # both the first-name line and the autofit surname must be allowed to break
    # so a long/space-less name can never clip off the canvas edge.
    raw = _raw()
...
def test_lifts_pack_diversity_above_prior_mark():
    # The Tier-A picker spreads a 10-card pack across the library by seed-modulo.
    # With ticker_strip the library is >= 8, so a seeds 0..9 pack spans >= 8
    # distinct archetypes -> diversity >= 0.8, strictly above the prior 0.70 mark.
    names = archetypes.list_archetypes()
    assert NAME in names
    pack = [archetypes.pick_archetype(s) for s in range(10)]
    div = VM.archetype_diversity(pack)
    assert div >= 0.8, f"expected pack diversity >= 0.8, got {div}"
