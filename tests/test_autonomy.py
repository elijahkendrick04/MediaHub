...
def test_unknown_tool_is_blocked_never_executed(tmp_path):
    ...
    out = dispatch(_ctx(env), AutonomyLevel.PREPARE, "run_shell", {"cmd": "rm -rf /"})
...
def test_level_gates_which_tools_run(tmp_path):
    ...
    out = dispatch(
        _ctx(env), AutonomyLevel.SUGGEST, "queue_for_approval", {"run_id": "r1", "card_ids": ["c1"]}
    )
    ...
    out2 = dispatch(
        _ctx(env), AutonomyLevel.SUGGEST, "draft_caption", {"run_id": "r1", "card_id": "c1"}
    )
...
def test_tools_for_level_never_exposes_above_level():
    assert {t["name"] for t in tools_for_level(AutonomyLevel.OFF)} == set()
    suggest = {t["name"] for t in tools_for_level(AutonomyLevel.SUGGEST)}
    assert "draft_caption" not in suggest and "queue_for_approval" not in suggest
    assert "queue_for_approval" not in {t["name"] for t in tools_for_level(AutonomyLevel.DRAFT)}
    assert "queue_for_approval" in {t["name"] for t in tools_for_level(AutonomyLevel.PREPARE)}
...
def test_cross_tenant_run_is_blocked(tmp_path):
    ...
    out = dispatch(_ctx(env, org="orgA"), AutonomyLevel.PREPARE, "list_cards", {"run_id": "r2"})
...
def test_path_like_run_id_is_just_an_unowned_id(tmp_path):
    ...
    env = make_env(tmp_path, workflow=wf)
    out = dispatch(
        _ctx(env), AutonomyLevel.SUGGEST, "get_run_summary", {"run_id": "../../etc/passwd"}
    )
...
def test_tools_never_mutate_the_deterministic_report(tmp_path):
    ...
    ctx = _ctx(env)
    dispatch(ctx, AutonomyLevel.PREPARE, "list_cards", {"run_id": "r1"})
    dispatch(
        ctx,
        AutonomyLevel.PREPARE,
        "draft_caption",
    ...
    dispatch(
        ctx, AutonomyLevel.PREPARE, "queue_for_approval", {"run_id": "r1", "card_ids": ["c1", "c2"]}
    )
...
def test_every_step_is_audited(tmp_path):
    ...
    ctx = _ctx(env)
    dispatch(ctx, AutonomyLevel.PREPARE, "list_cards", {"run_id": "r1"})
    dispatch(ctx, AutonomyLevel.PREPARE, "no_such_tool", {})
    entries = env.audit.read("orgA")
...
def test_off_by_default(monkeypatch, tmp_path):
    ...
    with pytest.raises(AutonomyDisabled):
        run_autonomy("orgA", "do it", AutonomyLevel.PREPARE, make_env(tmp_path))
...
def test_loop_is_bounded(monkeypatch, tmp_path):
    ...
    res = run_autonomy(
        "orgA", "prepare my pack", AutonomyLevel.DRAFT, make_env(tmp_path), max_steps=99
    )
...
def test_full_session_drafts_but_never_publishes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mediahub.ai_core.llm.ask_with_tools", fake_awt)
    res = run_autonomy("orgA", "prepare a post for the best swim", AutonomyLevel.PREPARE, env)
...
def test_injected_instruction_in_data_cannot_escalate(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mediahub.ai_core.llm.ask_with_tools", fake_awt)
    run_autonomy("orgA", "make great posts", AutonomyLevel.PREPARE, env)
...
def test_provider_not_configured_propagates(monkeypatch, tmp_path):
    ...
    with pytest.raises(ProviderNotConfigured):
        run_autonomy("orgA", "go", AutonomyLevel.SUGGEST, env)
