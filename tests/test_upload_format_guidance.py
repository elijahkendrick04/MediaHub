...
def upload_client(client):
    # gate bypassed so /upload renders (the ``client`` fixture sets TESTING=True)
    return client
...
class TestUploadFormatGuidance:
    ...
    def test_csv_mentioned_in_upload_page(self, upload_body):
        ...
        assert (
            ".csv" in upload_body or "CSV" in upload_body
    ...
    def test_sdif_mentioned_in_upload_page(self, upload_body):
        ...
        assert (
            "sdif" in body_lower or "sd3" in body_lower or "cl2" in body_lower
    ...
    def test_accept_attribute_includes_sdif(self, upload_body):
        ...
        assert (
            ".sdif" in upload_body or ".sd3" in upload_body
    ...
    def test_xlsx_upload_not_rejected_on_extension(self, upload_client):
        ...
        assert not (r.status_code == 400 and "isn't supported" in r.get_data(as_text=True))
