...
def _skip_if_no_pdf():
    return pytest.mark.skipif(not os.path.exists(PDF_PATH), reason="Sample PDF not found")
...
def test_parses_mism_2024_real_pdf():
    ...
    # Meet name correct
    assert meet.name.startswith(
        "ARENA Manchester International"
    ...
    assert len(unique_events) >= 30, f"Expected ≥30 events, got {len(unique_events)}"

    # Should find many swimmers
    assert len(meet.swimmers) >= 200, f"Expected ≥200 swimmers, got {len(meet.swimmers)}"

    # City of Manchester Aquatics should be present via club_name
    coma_swimmers = [
        s for s in meet.swimmers.values() if "Manch" in (getattr(s, "club_name", "") or "")
    ]
    assert len(coma_swimmers) >= 5, f"Expected ≥5 COMA swimmers, got {len(coma_swimmers)}"
...
def test_home_renders_without_profiles(client):
    ...
    The ``client`` fixture runs on an isolated DATA_DIR with an empty
    ``club_profiles`` dir (so no profile is seeded) and sets ``TESTING=True``,
    which bypasses the first-run organisation gate.
    """
    r = client.get("/")
...
def test_adapter_dispatcher_recognises_pdf():
    ...
    assert (
        log.chosen_adapter == "sportsystems_pdf"
