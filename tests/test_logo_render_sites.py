...
def _seed_profile(cp_module, *, brand_logo_url=None, primary="#06D6A0",
    ...
    prof.brand_kit = {
        "profile_id": pid, "display_name": "Logo Test Club",
        "primary_colour": primary, "secondary_colour": "#0E2A47",
    }
...
class TestDetectedLogoSite:
    ...
    def test_detected_logo_wrapped_in_chip(self, app_client):
        ...
        prof = _seed_profile(cp, brand_logo_url="https://example.com/logo.png",
                             primary="#06D6A0")
...
class TestProfileCardSite:
    ...
    def test_profile_card_logo_uses_chip(self, app_client):
        client, wm, cp = app_client
        prof = _seed_profile(
            cp, brand_logo_url="https://example.com/club.png",
            primary="#A30D2D",
        )
...
class TestRenderHelper:
    ...
    def test_force_chip_overrides_dominant(self, app_client):
        ...
        with _request_context(wm):
            html = wm._logo_chip_html(
                "/x.png", "alt",
                dominant_hex="#FFFFFF", surface_hex="#0A0B11",
                force_chip=True,
            )
            assert "mh-logo-chip" in html
    ...
    def test_alt_text_escaped(self, app_client):
        ...
        with _request_context(wm):
            ...
            # via alt text).
            assert "<img onerror=alert(1)>" not in html or \
                   "&lt;" in html
